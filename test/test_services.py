...
def test_classification_service_merges_model_output(monkeypatch):
    transformed = pd.DataFrame(
        {
            "publication_id": [str(i) for i in range(20)],
            "ventas_30d": list(range(20, 0, -1)),
            "visitas_30d": list(range(40, 20, -1)),
            "precio_actual": [float(i) for i in range(20)],
            "stock_actual": [1] * 20,
            "en_promocion": [0] * 20,
        }
    )
    ...
    monkeypatch.setattr(
        ...
        lambda data: {
            "hay_validos": True,
            "productos_validos": list(range(20)),
            "productos_invalidos": [
                {
                    "publication_id": "INVALID",
                    "errores": ["inconsistencia"],
                }
            ],
    ...
    monkeypatch.setattr(
        ...
        lambda X: (
            pd.DataFrame(
                {
                    "categoria": ["A"] * 4 + ["B"] * 6 + ["C"] * 10,
                    "score_inicial": [float(i) for i in range(20)],
                }
            ),
            {
    ...
    assert len(result["resultados"]) == result["productos_validos"]

    assert (
        sum(result["diagnostico"]["conteos_finales"].values())
        == 20
    )

    assert (
        len(result["resultados"])
        + len(result["productos_invalidos"])
        == 21
    )
...
def test_classification_service_requires_twenty_valid_rows_after_exclusions(
    ...
    monkeypatch.setattr(
        ...
        lambda data: {
            "hay_validos": True,
            "productos_validos": list(range(19)),
            "productos_invalidos": [
                {
                    "publication_id": "INVALID",
                    "errores": ["inconsistencia"],
    ...
    def fail_if_called(X):
        ...
        raise AssertionError("SS-EKMeans no debe ejecutarse")
    ...
    assert result["minimo_operacional"] == 20
    assert result["resultados"] == []
    assert "después de excluir" in result["mensaje"]


def test_optimization_service_requires_twenty_valid_rows(monkeypatch):
    monkeypatch.setattr(
    ...
    result = optimization_service.OptimizationService().optimize(
        object()
    )
...
def test_surrogate_training_inherits_operational_minimum(monkeypatch):
    monkeypatch.setattr(
        explainability_service,
        "ejecutar_clasificacion",
        lambda data: {
            "mensaje": "Se obtuvieron 19 publicaciones válidas.",
    ...
    def fail_if_called(*args, **kwargs):
        ...
        worker_called = True
        raise AssertionError("El worker no debe entrenarse")
    ...
    result = explainability_service.entrenar_surrogate_autosklearn(
        object()
    )

    assert worker_called is False
    assert result["clasificacion"]["productos_validos"] == 19
    assert result["clasificacion"]["minimo_operacional"] == 20
    assert "No hay resultados válidos" in result["mensaje"]
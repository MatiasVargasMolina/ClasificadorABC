...
def _mean_pairwise_ari(labels_by_seed: Sequence[pd.Series]) -> float:
...
def _mean_exact_agreement(
    ...
    agreement_values = [
        float((first_labels == second_labels).mean() * 100)
        for first_labels, second_labels in combinations(
            labels_by_seed,
            2,
        )
...
def optimize_sse_kmeans(
    ...
    if n_trials < 1:
        raise ValueError("n_trials debe ser mayor o igual a 1.")

    seeds = tuple(int(seed) for seed in optimization_seeds)

    if not seeds:
        raise ValueError(
            "Debe proporcionarse al menos una semilla de optimización."
        )
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        for seed in seeds:
            ...
            try:
                ...
                trial.set_user_attr(
                    "error",
                    f"Semilla {seed}: {error}",
                )
                raise optuna.TrialPruned(
                    f"La configuración falló con la semilla {seed}."
                ) from error
            ...
            if labels.nunique() < 2:
                trial.set_user_attr(
                    "error",
                    f"Semilla {seed}: se obtuvo menos de dos grupos.",
                )
                raise optuna.TrialPruned(
                    "La configuración no generó suficientes grupos."
                )
            ...
            iteration_values.append(float(model.n_iter_ or 0))
            converged_run_values.append(
                float(model.converged_runs_)
            )
            discarded_run_values.append(
                float(model.discarded_runs_)
            )
            labels_by_seed.append(labels)

        mean_silhouette = _mean(silhouettes)
        ...
        trial.set_user_attr(
            "davies_bouldin_mean",
            _mean(davies_bouldin_values),
        )
        trial.set_user_attr(
            "davies_bouldin_std",
            _std(davies_bouldin_values),
        )
        trial.set_user_attr(
            "calinski_harabasz_mean",
            _mean(calinski_harabasz_values),
        )
        trial.set_user_attr(
            "calinski_harabasz_std",
            _std(calinski_harabasz_values),
        )
        trial.set_user_attr(
            "inertia_mean",
            _mean(inertias),
        ...
        trial.set_user_attr(
            ...
            _mean(normalized_inertias),
        )
        trial.set_user_attr(
            "elapsed_seconds_mean",
            _mean(elapsed_values),
        )
        trial.set_user_attr(
        ...
        trial.set_user_attr(
            "iterations_mean",
            _mean(iteration_values),
        )
        trial.set_user_attr(
            "converged_runs_mean",
            _mean(converged_run_values),
        )
        trial.set_user_attr(
            "discarded_runs_mean",
            _mean(discarded_run_values),
        )
        trial.set_user_attr(
            "pairwise_ari_mean",
            _mean_pairwise_ari(labels_by_seed),
        )
        trial.set_user_attr(
            "exact_agreement_mean_pct",
            _mean_exact_agreement(labels_by_seed),
        )
    ...
    study = optuna.create_study(
        direction="maximize",
        sampler=sampler,
        study_name="sse_kmeans_internal_quality",
    )
    ...
    if not complete_trials:
        raise RuntimeError(
            "Optuna no completó ningún trial válido. "
            "Revisa los errores registrados en las corridas podadas."
        )
    ...
    best_params = {
        ...
        "max_iter": MAX_ITER,
        "tol": float(best_trial.params["tol"]),
    ...
    return {
        ...
        "optimization_config": {
            ...
            "objective": "mean_silhouette",
            "n_init_options": list(N_INIT_OPTIONS),
            "tol_min": 1e-6,
            "tol_max": 1e-2,
            "max_iter_fixed": MAX_ITER,
            "shuffle_unlabeled_fixed": SHUFFLE_UNLABELED,
        },
        "trials": [
            {
